...
- local package / SDK
- local CLI for keygen, evaluation, receipt issuance, gate checks, and verification
- built-in risk presets plus exportable policy JSON files
- OpenClaw hook workflow example with file-based artifacts
- signed approval receipt format
- policy rules for high-risk onchain actions
- verification utility

## Current milestone
로컬 reference primitive를 넘어서, 사람이 바로 설치하고 실행해볼 수 있는 approval operations kit으로 올렸다.

- zero-hosting, zero-ops local workflow 유지
- OpenClaw가 붙일 수 있는 preflight / approval / gate 예제 경로 제공
- receipt가 단순히 액션 digest만 맞는지뿐 아니라 현재 policy decision과도 맞는지 검사
